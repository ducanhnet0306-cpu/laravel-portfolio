...
<section id="top" class="relative overflow-hidden bg-hero-radial">
    <div class="container-page grid items-center gap-12 pb-20 pt-16 sm:pt-24 lg:grid-cols-12 lg:pb-28 lg:pt-28">
        ...
        <div class="lg:col-span-7 reveal">
            ...
            <p class="mt-6 max-w-2xl text-lg text-slate-600 sm:text-xl text-balance">
            ...
            <dl class="mt-12 grid max-w-xl grid-cols-2 gap-4 sm:grid-cols-4">
                @foreach (($portfolio->about['highlights'] ?? []) as $stat)
                    <div class="rounded-2xl border border-slate-100 bg-white/70 px-4 py-3 text-center backdrop-blur">
                        <dt class="text-[11px] uppercase tracking-wider text-slate-500">{{ $stat['label'] }}</dt>
        ...
        <div class="lg:col-span-5 reveal">
            <div class="relative mx-auto aspect-square w-full max-w-md">
                <div class="absolute inset-0 -z-10 rounded-[40%] bg-gradient-to-br from-brand-200 via-brand-100 to-white blur-2xl"></div>

                <div class="relative h-full w-full overflow-hidden rounded-[2.5rem] border border-white/60 bg-white/80 p-2 shadow-soft backdrop-blur">
                ...
                <div class="absolute -left-4 top-10 hidden rounded-2xl border border-slate-100 bg-white px-4 py-3 shadow-soft sm:block">
                    <p class="text-[10px] uppercase tracking-wider text-slate-500">Hiện đang ở</p>
                    <p class="text-sm font-semibold text-slate-900">{{ $portfolio->owner['location'] ?? '' }}</p>
                </div>

                <div class="absolute -right-4 bottom-10 hidden rounded-2xl border border-slate-100 bg-white px-4 py-3 shadow-soft sm:block">
                    <p class="text-[10px] uppercase tracking-wider text-slate-500">Sẵn sàng cho</p>
